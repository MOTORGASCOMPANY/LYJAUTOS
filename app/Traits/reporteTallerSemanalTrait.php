...
trait reporteTallerSemanalTrait
{
    public function procesar()
    {
        ...
        $tabla3 = $this->filtrarExternos($tabla2);
        $aux = $this->agruparTalleresConsolidar($tabla3);
        $semanales = $this->filtrarPorFrecuencia($aux, 'es_semanal');
        return [
            //'aux' => $aux,
            'semanales' => $semanales,
        ];
    ...
    private function agruparTalleresConsolidar($tabla)
    {
        ...
        return $tabla
            ...
            ->map(function ($items) {
                ...
                // Agrupar IDs por tipo de modelo
                $idsPorModelo = $items
                    ->groupBy('tipo_modelo')
                    ->map(function ($items, $modelo) {
                        return "{$modelo} => " . $items->pluck('id')->implode(', ');
                    })
                    ->values()
                    ->all();
                ...
                return [
                    'taller' => $items->first()['taller'],
                    'total'  => $items->sum(fn ($item) => (float) $item['precio']),
                    'ids'    => $idsPorModelo,
                ];
            })
            ->filter(fn ($data) => $data['total'] > 0)
