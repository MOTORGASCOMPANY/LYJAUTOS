...
{
    // VISTA Y DESCARGA FICHAS TECNICAS GNV ✅
    ...
    public function generarCheckListArribaGnv($idCert)
    {
        if (Certificacion::findOrFail($idCert)) {
            ...
            $data = [
                ...
                "fecha" => $certificacion->fecha_documento->format('d/m/Y'),
        ...
        if (Certificacion::findOrFail($id)) {
            ...
            if ($certificacion->Servicio->tipoServicio->id) {
                if ($certificacion->Servicio->tipoServicio->id == 5) {
                    ...
                    // Genera el código QR
                    $urlDelDocumento = 'www.ljautosperu.com' . route('verPdfModificacion', $id, false); // www.motorgasperu.com
                    ...
                    return $pdf->stream($certificacion->Vehiculo->placa . '-' . $hoja->numSerie . '-modificacion.pdf');
                }
                return abort(404);
            }
        } else {
        ...
        if (Certificacion::findOrFail($id)) {
            ...
            if ($certificacion->Servicio->tipoServicio->id) {
                if ($certificacion->Servicio->tipoServicio->id == 5) {
                    ...
                    $certificacion->load('Vehiculo.modificaciones');
                    // Genera el código QR
                    $urlDelDocumento = 'www.ljautosperu.com' . route('verPdfModificacion', $id, false);
                    $qrCode = QrCode::size(70)->generate($urlDelDocumento);
                    $data = [
                        "fecha" => $fecha,
                        //"empresa" => "MOTORGAS COMPANY S.A.",
        ...
        if (Certificacion::findOrFail($id)) {
            ...
            if ($certificacion->Servicio->tipoServicio->id) {
                if ($certificacion->Servicio->tipoServicio->id == 1 || $certificacion->Servicio->tipoServicio->id == 10) {
                    $meses = array("Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre");
                    //$fechaCert = $certificacion->created_at;
                    $fechaCert = $certificacion->fecha_documento; //fecha_documento esta casteado es fechaCertificado
                    $fecha = $fechaCert->format('d') . ' días del mes de ' . $meses[$fechaCert->format('m') - 1] . ' del ' . $fechaCert->format('Y') . '.';
                    $chip = $certificacion->Vehiculo->Equipos->where("idTipoEquipo", 1)->first();
                    $equipos = $certificacion->Vehiculo->Equipos->where("idTipoEquipo", "!=", 1)->sortBy("idTipoEquipo");
                    $hoja = $certificacion->Materiales->where('idTipoMaterial', 1)->first();
                    // Genera el código QR
                    $urlDelDocumento = 'www.ljautosperu.com' . route('verPdfInicial', $id, false); // Reemplaza 'certificadoAnualGnv' con el nombre correcto de tu ruta
                    $qrCode = QrCode::size(70)->generate($urlDelDocumento);
                    $data = [
                        "fecha" => $fecha,
                        //"empresa" => "MOTORGAS COMPANY S.A.",
    ...
    public function descargarPdfInicialGnv($id)
    {
        if (Certificacion::findOrFail($id)) {
            ...
            $hoja = $certificacion->Materiales->where('idTipoMaterial', 1)->first();
            // Genera el código QR
            $urlDelDocumento = 'www.ljautosperu.com' . route('verPdfInicial', $id, false);
            $qrCode = QrCode::size(70)->generate($urlDelDocumento);
            $data = [
                "fecha" => $fecha,
                //"empresa" => "MOTORGAS COMPANY S.A.",
        ...
        if (Certificacion::findOrFail($id)) {
            ...
            if ($certificacion->Servicio->tipoServicio->id) {
                if ($certificacion->Servicio->tipoServicio->id == 2) {
                    ...
                    $hoja = $certificacion->Materiales->where('idTipoMaterial', 1)->first();
                    // Genera el código QR
                    $urlDelDocumento = 'www.ljautosperu.com' . route('verPdfAnual', $id, false); // Reemplaza 'certificadoAnualGnv' con el nombre correcto de tu ruta
                    $qrCode = QrCode::size(70)->generate($urlDelDocumento);
                    $data = [
                        "fecha" => $fecha,
                        //"empresa" => "MOTORGAS COMPANY S.A.",
    ...
    public function descargarPdfAnualGnv($id)
    {
        if (Certificacion::findOrFail($id)) {
            ...
            $hoja = $certificacion->Materiales->where('idTipoMaterial', 1)->first();
            // Genera el código QR
            $urlDelDocumento = 'www.ljautosperu.com' . route('verPdfAnual', $id, false);
            $qrCode = QrCode::size(70)->generate($urlDelDocumento);
            $data = [
                "fecha" => $fecha,
                //"empresa" => "MOTORGAS COMPANY S.A.",
    ...
    function completarConCeros($numero, $longitudDeseada = 5) //$longitudDeseada = 7
    {
    ...
    // VISTA Y DESCARGA INICIALES GLP ✅
        ...
        if (Certificacion::findOrFail($id)) {
            ...
            if ($certificacion->Servicio->tipoServicio->id) {
                if ($certificacion->Servicio->tipoServicio->id == 3) {
                    ...
                    $cargaUtil = $this->calculaCargaUtil($certificacion->Vehiculo->pesoBruto, $certificacion->Vehiculo->pesoNeto);
                    // Genera el código QR
                    $urlDelDocumento = 'www.ljautosperu.com' . route('verPdfInicialGlp', $id, false); // Reemplaza 'certificadoAnualGnv' con el nombre correcto de tu ruta
                    $qrCode = QrCode::size(70)->generate($urlDelDocumento);
                    $data = [
                        "fecha" => $fecha,
                        "cargaUtil" => $cargaUtil,
    ...
    //Vista y descarga de PRE-CONVERSIONES GNV (PREINICIALES) ✅
        ...
        if (Certificacion::findOrFail($id)) {
            ...
            if ($certificacion->Servicio->tipoServicio->id) {
                if ($certificacion->Servicio->tipoServicio->id == 12) {
                    ...
                    $hoja = $certificacion->Materiales->where('idTipoMaterial', 1)->first();
                    // Genera el código QR
                    $urlDelDocumento = 'www.ljautosperu.com' . route('verPdfPreGnvPdf', $id, false);
                    $qrCode = QrCode::size(70)->generate($urlDelDocumento);
                    $data = [
                        "fecha" => $fecha,
                        //"empresa" => "MOTORGAS COMPANY S.A.",
    ...
    //Vista de PRE-CONVERSIONES GLP (PREINICIALES) ✅
        ...
        if (Certificacion::findOrFail($id)) {
            ...
            if ($certificacion->Servicio->tipoServicio->id) {
                if ($certificacion->Servicio->tipoServicio->id == 13)
                {
                    ...
                    $cargaUtil = $this->calculaCargaUtil($certificacion->Vehiculo->pesoBruto, $certificacion->Vehiculo->pesoNeto);
                    // Genera el código QR
                    $urlDelDocumento = 'www.ljautosperu.com' . route('verPdfPreInicialGlp', $id, false);
                    $qrCode = QrCode::size(70)->generate($urlDelDocumento);
                    $data = [
                        "fecha" => $fecha,
                        //"empresa" => "MOTORGAS COMPANY S.A.",
        ...
        if (Certificacion::findOrFail($id)) {
            ...
            if ($duplicado->Servicio->tipoServicio->id) {
                if ($antiguo->Servicio->tipoServicio->id == 2) {
                    ...
                    $fechaCert = $duplicado->fecha_documento;
                    //$fechaAntiguo = $antiguo->created_at;
                    $fechaAntiguo = $antiguo->fecha_documento;
                    $fecha = $fechaCert->format('d') . ' días del mes de ' . $meses[$fechaCert->format('m') - 1] . ' del ' . $fechaCert->format('Y') . '.';
                    $hoja = $duplicado->Hoja;
                    $hojaAntiguo = $antiguo->Hoja;
                    // Genera el código QR
                    $urlDelDocumento = 'www.ljautosperu.com' . route('verPdfAnualDupli', $id, false);
                    $qrCode = QrCode::size(70)->generate($urlDelDocumento);
                    $data = [
                        "fecha" => $fecha,
                        //"empresa" => "MOTORGAS COMPANY S.A.",
                        ...
                        "numSerie" => $this->completarConCeros($duplicado->numSerie),
                        "anioSerie" => $duplicado->anioSerie,
                        "hoja" => $hoja,
                        "fechaCert" => $fechaCert,
        ...
        if (Certificacion::findOrFail($id)) {
            ...
            if ($antiguo->Servicio->tipoServicio->id) {
                if ($antiguo->Servicio->tipoServicio->id == 1) {
                    ...
                    $hojaAntiguo = $antiguo->Hoja;
                    // Genera el código QR
                    $urlDelDocumento = 'www.ljautosperu.com' . route('verPdfInicialDupli', $id, false);
                    $qrCode = QrCode::size(70)->generate($urlDelDocumento);
    ...
    public function generaDuplicadoExternoAnualGnv($id)
    {
        if (Certificacion::findOrFail($id)) {
            ...
            if ($dupli->servicio == 2) {
                ...
                //$hojaAntiguo=$antiguo->Hoja;
                // Genera el código QR
                $urlDelDocumento = 'www.ljautosperu.com' . route('verPdfAnualDupliExt', $id, false);
                $qrCode = QrCode::size(70)->generate($urlDelDocumento);
    ...
    public function generaDuplicadoExternoInicialGnv($id)
    {
        if (Certificacion::findOrFail($id)) {
            ...
            if ($dupli->servicio == 1) {
                ...
                $hoja = $duplicado->Hoja;
                // Genera el código QR
                $urlDelDocumento = 'www.ljautosperu.com' . route('verPdfInicialDupliExt', $id, false);
                $qrCode = QrCode::size(70)->generate($urlDelDocumento);
    ...
    //VISTA Y DESCARGA DE FICHA EVALUACION PRECONVERSION GNV ✅
    public function generarPreConversionGnv($id)
    {
        if (Certificacion::findOrFail($id)) {
            $certificacion = Certificacion::find($id);
            if ($certificacion->Servicio->tipoServicio->id) {
                if ($certificacion->Servicio->tipoServicio->id == 1 || $certificacion->Servicio->tipoServicio->id == 10 || $certificacion->Servicio->tipoServicio->id == 12 || 
                    $certificacion->Servicio->tipoServicio->id == 3 || $certificacion->Servicio->tipoServicio->id == 13 || $certificacion->Servicio->tipoServicio->id == 14) {
                    $chip = $certificacion->vehiculo->Equipos->where("idTipoEquipo", 1)->first();
                    $equipos = $certificacion->vehiculo->Equipos->where("idTipoEquipo", "!=", 1)->sortBy("idTipoEquipo");
                    //dd($equipos);
                    //$hoja = $certificacion->Materiales->where('idTipoMaterial', 1)->first();
                    $hoja = $certificacion->Materiales->whereIn('idTipoMaterial', [1, 3])->first();
                    //$fechaCert = $certificacion->created_at;
                    $fechaCert = $certificacion->fecha_documento;
                    $fec = $fechaCert->format("d/m/Y");
                    $data = [
    ...
    public function descargarPreConversionGnv($id)
    {
        if (Certificacion::findOrFail($id)) {
            $certificacion = Certificacion::find($id);
            if ($certificacion->Servicio->tipoServicio->id) {
                if ($certificacion->Servicio->tipoServicio->id == 1 || $certificacion->Servicio->tipoServicio->id == 10 || $certificacion->Servicio->tipoServicio->id == 14) {
                    $chip = $certificacion->vehiculo->Equipos->where("idTipoEquipo", 1)->first();
                    $equipos = $certificacion->vehiculo->Equipos->where("idTipoEquipo", "!=", 1)->sortBy("idTipoEquipo");
                    //dd($equipos);
                    $hoja = $certificacion->Materiales->where('idTipoMaterial', 1)->first();
    ...
    // GENERAR CARGO ASIGNACION MATERIALES ✅
    ...
    public function generaPdfContrato($id)
    {
        ...
        if ($contrato) {
            ...
            $data = [
                ...
                'dniEmpleado' => $contrato->empleado->dni,
                'domicilioEmpleado' => $contrato->empleado->domicilio,
    ...
    public function generaPdfContratoPlanilla($id)
    {
        $contrato = ContratoTrabajo::findOrFail($id);
        if ($contrato) {
            $usuario = User::findOrFail($contrato->idUser);
            ...
            $duracion = $fechaCertInicio->diff($fechaCert2);
            $duracionTexto = '';
            if ($duracion->y > 0) {
                $duracionTexto .= $duracion->y . ' año' . ($duracion->y > 1 ? 's' : '') . ', ';
            }
            if ($duracion->m > 0) {
                $duracionTexto .= $duracion->m . ' mes' . ($duracion->m > 1 ? 'es' : '') . ', ';
            }
            if ($duracion->d > 0) {
                $duracionTexto .= $duracion->d . ' día' . ($duracion->d > 1 ? 's' : '');
            }

        // Elimina la coma final si no hay días
        $duracionTexto = rtrim($duracionTexto, ', ');
            ...
            $pdf = App::make('dompdf.wrapper');
            $pdf->loadView('contratoPlanilla', $data);
            return $pdf->stream($contrato->id . '-contrato.pdf');
        } else {
            abort(404);
        }
    ...
    public function generaPdfBoleta($id)
    {
        ...
        // Agregar url temporal o local a cada documento
        foreach ($documentos as $doc) {
            if ($doc->migrado == 1) {
                $doc->url = Storage::disk('do')->temporaryUrl($doc->ruta, now()->addMinutes(5));
            } else {
                $doc->url = public_path('storage/docsBoletas/' . basename($doc->ruta));
            }
        }
    ...
    //Genera pdf comprobante formatos al contado
    public function generaPdfContado($id)
    {
        // Encuentra el registro de Contado
        $contado = Contado::findOrFail($id);
        // Obtén los documentos relacionados
        $documentos = $contado->contadoarchivo;
        ...
        foreach ($documentos as $doc) {
            if ($doc->migrado == 1) {
                ...
                $doc->url = public_path('storage/docsContados/' . basename($doc->ruta));
                //public_path('storage/' . $doc->ruta);
            }
        }

        // Obtén los números de serie usando relaciones
        $numSeries = $contado->salida->materiales->pluck('numSerie');
        // Agrupar los materiales por tipo y obtener el menor y mayor numSerie
        $materialesAgrupados = $contado->salida->materiales
        ->groupBy('idTipoMaterial')
        ->map(function ($materiales) {
            // Filtra los numSerie no vacíos
            $series = $materiales->pluck('numSerie')
            ->filter(fn($serie) => trim($serie) !== '')
            ->sort()
            ->values();
            return [
                'tipo' => $materiales->first()->tipo->descripcion ?? 'Sin Tipo',
                //'menor' => $materiales->min('numSerie'),
                //'mayor' => $materiales->max('numSerie'),
                'menor' => $series->isNotEmpty() ? $series->first() : 'Sin Serie',
                'mayor' => $series->isNotEmpty() ? $series->last() : 'Sin Serie',
        ...
        $pdf = App::make('dompdf.wrapper');
        $pdf->loadView('contadosimage', $data);
        return $pdf->stream('contado-' . $id . '.pdf');
    }
    ...
    public function generaPdfCerTaller($id)
    {
        ...
        $urlDelDocumento = route('verPdfCerTaller', $id, true);      
        $qrCode = QrCode::size(70)->generate($urlDelDocumento);
        $data = [
            "certi" => $certi,
            "qrCode" => $qrCode,
        ...
        // Devolver el PDF como respuesta con encabezados manuales
        /*return response($pdf->output(), 200)
            ->header('Content-Type', 'application/pdf')
            ->header(
                'Content-Disposition',
                'inline; filename="certificado_taller_' . $id . '.pdf"'
        );*/
    }
        ...
        if (Certificacion::findOrFail($id)) {
            ...
            if ($duplicado->Servicio->tipoServicio->id) {
                if ($antiguo->Servicio->tipoServicio->id == 4) {
                    ...
                    $hojaAntiguo = $antiguo->Hoja;
                    // Genera el código QR
                    $urlDelDocumento = 'www.ljautosperu.com' . route('verPdfAnualDupliGlp', $id, false); // cambiar la ruta para QR
                    $qrCode = QrCode::size(70)->generate($urlDelDocumento);
        ...
        if (Certificacion::findOrFail($id)) {
            ...
            if ($antiguo->Servicio->tipoServicio->id) {
                if ($antiguo->Servicio->tipoServicio->id == 3) {
                    ...
                    $hojaAntiguo = $antiguo->Hoja;
                    // Genera el código QR
                    $urlDelDocumento = 'www.ljautosperu.com' . route('verPdfInicialDupliGlp', $id, false); // cambiar la ruta para QR
                    $qrCode = QrCode::size(70)->generate($urlDelDocumento);
    ...
    //vista y descarga de DUPLICADOS EXTERNOS ANUALES GLP
    public function generaDuplicadoExternoAnualGlp($id)
    {
        if (Certificacion::findOrFail($id)) {
            ...
            if ($dupli->servicio == 4) {
                ...
                $hoja = $duplicado->Hoja;
                // Genera el código QR
                $urlDelDocumento = 'www.ljautosperu.com' . route('verPdfAnualDupliExtGlp', $id, false); // cambiar la ruta para QR
                $qrCode = QrCode::size(70)->generate($urlDelDocumento);
                $equipos = $duplicado->Vehiculo->Equipos->where("idTipoEquipo", ">", 3)->sortBy("idTipoEquipo");
                $cargaUtil = $this->calculaCargaUtil($duplicado->Vehiculo->pesoBruto, $duplicado->Vehiculo->pesoNeto);
    ...
    //vista y descarga de DUPLICADOS EXTERNOS INICIALES GLP
    public function generaDuplicadoExternoInicialGlp($id)
    {
        if (Certificacion::findOrFail($id)) {
            ...
            if ($dupli->servicio == 3) {
                ...
                $hoja = $duplicado->Hoja;
                // Genera el código QR
                $urlDelDocumento = 'www.ljautosperu.com' . route('verPdfInicialDupliExtGlp', $id, false);
                $qrCode = QrCode::size(70)->generate($urlDelDocumento);
    ...
    //vista y descarga de  inicial gnv overhul ✅
    public function generaPdfInicialGnvOverhul($id)
    {
        if (Certificacion::findOrFail($id)) {
            $certificacion = Certificacion::find($id);
            if ($certificacion->Servicio->tipoServicio->id) {
                if ($certificacion->Servicio->tipoServicio->id == 14) {
                    $meses = array("Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre");
                    //$fechaCert = $certificacion->created_at;
                    $fechaCert = $certificacion->fecha_documento;
                    $fecha = $fechaCert->format('d') . ' días del mes de ' . $meses[$fechaCert->format('m') - 1] . ' del ' . $fechaCert->format('Y') . '.';
                    ...
                    $hoja = $certificacion->Materiales->where('idTipoMaterial', 1)->first();
                    // Genera el código QR
                    $urlDelDocumento = 'www.ljautosperu.com' . route('verPdfInicialOverhul', $id, false);
                    $qrCode = QrCode::size(70)->generate($urlDelDocumento);
                    $data = [
                        "fecha" => $fecha,
                        //"empresa" => "MOTORGAS COMPANY S.A.",
                    ...
                    $pdf = App::make('dompdf.wrapper');
                    $pdf->loadView('conversionGnvOverhul', $data);
                    //return $pdf->stream($id.'-'.date('d-m-Y').'-cargo.pdf');
                    return  $pdf->stream($certificacion->Vehiculo->placa . '-' . $hoja->numSerie . '-inicialOverhul.pdf');
                }
                return abort(404);
            }
        } else {
    ...
    public function descargarPdfInicialGnvOverhul($id)
    {
        if (Certificacion::findOrFail($id)) {
            $certificacion = Certificacion::find($id);
            $meses = array("Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre");
            ...
            $pdf = App::make('dompdf.wrapper');
            $pdf->loadView('conversionGnvOverhul', $data);
            return  $pdf->download($certificacion->Vehiculo->placa . '-' . $hoja->numSerie . '-inicialOverhul.pdf');
        } else {
            return abort(404);
        }
    }
    ...
    // Genra pdf para planilla
    public function generaPdfPlanilla($periodo)
    {
        /*$detalles = PlanillaDetalle::with('contrato.empleado')
            ->where('periodo', $periodo)
            ->get();*/
        $detalles = PlanillaDetalle::with('contrato.empleado', 'usuario')  // incluir la relación con User
            ->where('periodo', $periodo)
            ->get()
            //->sortBy(fn($detalle) => optional($detalle->contrato->empleado)->name) // ordenar alfabéticamente
            ->sortBy(function ($detalle) {
                // tomamos el nombre desde contrato->empleado o desde usuario
                return mb_strtolower(
                    optional(optional($detalle->contrato)->empleado)->name
                    ?? optional($detalle->usuario)->name
                    ?? ''
                );
            })
            ->values(); // reindexar

        $total = $detalles->sum('total_pago');

        $pdf = App::make('dompdf.wrapper');
        $pdf->loadView('planilla', compact('detalles', 'total', 'periodo'))
                  ->setPaper('a4', 'landscape');

        return $pdf->stream("planilla_{$periodo}.pdf");
    }

    // Genera pdf resumen vacaciones
    public function generaPdfresumenVacaciones()
    {
        // Traemos todos los contratos con el empleado y sus vacaciones
        $contratos = ContratoTrabajo::with(['empleado', 'vacaciones.solicitudes'])
        ->where(function ($q) {
        ...
        $pdf = App::make('dompdf.wrapper');
        $pdf->loadView('resumen-vacaciones', compact('contratos'))
        ->setPaper('a4', 'portrait');

        return $pdf->stream("resumen_vacaciones.pdf");
    }
