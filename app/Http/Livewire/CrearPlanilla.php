...
class CrearPlanilla extends Component
{
    ...
    public function save()
    {
        ...
        DB::transaction(function () {
            foreach ($this->editable as $row) {
                ...
                // Determinar inspector
                if (!empty($row['user_id'])) {
                    $idInspector = $row['user_id'];
                } else {
                    $contrato = ContratoTrabajo::with('empleado')->find($row['contrato_id']);
                    $idInspector = optional($contrato->empleado)->id;
                }

                // Obtener número de cuenta
                $user = \App\Models\User::find($idInspector);
                $numeroCuenta = $user?->numero_cuenta;
                $beneficios = $user?->beneficios;

                // Buscar taller
                $taller = $this->obtenerTallerSemanaAnterior($idInspector);
    ...
    private function obtenerTallerSemanaAnterior($idInspector)
    {
        $start = Carbon::now()->subWeek()->startOfWeek();
        $end = Carbon::now()->subWeek()->endOfWeek();

        $certificacion = \App\Models\Certificacion::where('idInspector', $idInspector)
            ->whereBetween('created_at', [$start, $end])
            ->orderByDesc('created_at')
            ->first();

        //return $certificacion?->idTaller; // Devuelve null si no hay registro
        return $certificacion && $certificacion->Taller
            ? $certificacion->Taller->nombre   // ← devuelve nombre del taller
            : null;
