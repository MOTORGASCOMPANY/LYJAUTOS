    ...
    public function generarDocumentos()
    {
        $query = "
            SELECT
                ...
                tde.nombreTipo AS documento_faltante
            FROM contrato_trabajo ct
            JOIN users u ON u.id = ct.idUser
            CROSS JOIN tipodocumentoempleado tde
            WHERE NOT EXISTS (
                SELECT 1
                FROM documentoempleado de
                INNER JOIN documentoempleado_user deu ON deu.idDocumentoEmpleado = de.id
                WHERE deu.idUser = ct.id       -- <-- en tu esquema, deu.idUser = id del contrato
                AND de.tipoDocumento = tde.id
            )
        ";

        // Aplicar filtros dinámicamente (por empleado o por tipo de documento)
        $bindings = [];
        if (!empty($this->empleado)) {
            $query .= " AND ct.idUser = ?";
            $bindings[] = $this->empleado;
