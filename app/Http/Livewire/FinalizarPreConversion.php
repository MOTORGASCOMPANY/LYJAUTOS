...
class FinalizarPreConversion extends Component
{
    ...
    //para nuevo precio 2da certificacion
    public $nuevoPrecio;
    ...
    public function mount(){
        ...
        $this->tipoServicio = TipoServicio::find($tipoServicioId)->id;
    }
        ...
        if($this->conChip==1){
            if($chip!=null){               
                if(isset($this->vehiculo)){
                    ...
                    // Duplicar la certificación
                    $newCertificacion = $this->certificacion->replicate();
                    $newCertificacion->precio = $this->nuevoPrecio;
                    $newCertificacion->save();

                    // Buscar el idMaterial del registro de ServicioMaterial asociado a la certificación original
                    $servicioMaterialOriginal = ServicioMaterial::where('idCertificacion', $this->certificacion->id)->first();
                    if ($servicioMaterialOriginal) {
                        // Insertar un nuevo registro en ServicioMaterial con el id de la nueva certificación
                        ServicioMaterial::create([
                            'idCertificacion' => $newCertificacion->id,
                            'idMaterial' => $servicioMaterialOriginal->idMaterial,
                        ]);
                    }

                    //  Insertar en ServicioMaterial el id del chip con el id de la nueva certificación
                    ServicioMaterial::create([
                        'idCertificacion' => $newCertificacion->id,
                        'idMaterial' => $chip->id,
                    ]);
                    ...
                    $expe=Expediente::create([
                        ...
                        "certificado"=>$newCertificacion->Hoja->numSerie,
                        "estado"=>1,
                        "idTaller"=>$newCertificacion->Taller->id,
                        'usuario_idusuario'=>Auth::id(),
                        'servicio_idservicio'=>$newCertificacion->Servicio->id,
                    ]);           
                    $this->guardarFotos($expe);
                    guardarArchivosEnExpediente::dispatch($expe,$newCertificacion);                       
                    $certEx=CertifiacionExpediente::create(["idCertificacion"=>$newCertificacion->id,"idExpediente"=>$expe->id]);
    ...
    public function salvaTanque(){
        ...
        $this->open=false;
        $this->emit("minAlert",["titulo"=>"BUEN TRABAJO!","mensaje"=>"El ".$this->equipo->tipo->nombre." con serie ".$this->equipo->numSerie." se registro correctamente","icono"=>"success"]);        
        $this->reset(["equipo"]);   
    }
