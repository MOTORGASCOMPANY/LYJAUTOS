    ...
    public function mount()
    {
        ...
        $this->notificacionesPendientes = $this->user->notifications->map(function ($notification) {
            ...
            switch ($notification->type) {
                case 'App\Notifications\SolicitudEliminacion':
                    // Solo si no existe la certificación (ya fue eliminada)
                    $certificacion = isset($data['idServicio']) ? Certificacion::find($data['idServicio']) : null;
                    if (!$certificacion && isset($data['idEliminacion'])) {
                        // Certificación fue eliminada, buscar en tabla eliminación
                        $eliminacion = Eliminacion::find($data['idEliminacion']);
                        if ($eliminacion) {
                            $placa = $eliminacion->placa;
                            //$numero = $eliminacion->numSerie;
                            $numero = $eliminacion->numSerieMaterial;
                        }
                    } else {
                        // Certificación aún existe, usar su información
                        $placa = $data['placa'] ?? $certificacion?->placa;
                        $numero = $data['numSerie'] ?? $certificacion?->Hoja?->numSerie;
                    }
                    break;
