...
class ReporteCalcularGasol extends Component
{
    ...
    public $otros; // nuevo para taller no es diario ni semanal
    ...
    {
        $totalExternos = $this->aux ? $this->aux->flatten(1)->sum('precio') : 0;
        $totalSemanales = collect($this->semanales)->sum('total');
        $totalDiarios   = collect($this->diarios)->sum('total');
        $totalOtros     = collect($this->otros)->sum('total');
        $totalContados  = collect($this->tablaFinalContados)->sum('total_precio');

        $this->totalVentas = $totalExternos + $totalSemanales + $totalDiarios + $totalOtros + $totalContados;
    }
    ...
    /*public function aplicarFiltros($datos)
    {
        ...
        $registrosExternos = $datos->filter(
            fn($item) =>
                ...
                in_array($item['servicio'], $serviciosFiltrados) &&
                $item['externo'] == 1
                //(in_array($item['tipo_modelo'], $modelosExternoRequerido) && $item['externo'] == 1 || $item['tipo_modelo'] === $modeloSinFiltroExterno)
        );
        ...
        //$registrosAdicionalesFiltrados = $registrosAdicionalesFiltrados->reject(fn($item) => $registrosExternos->contains('id', $item['id']));
