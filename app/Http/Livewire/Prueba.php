...
class Prueba extends Component
{
    ...
    public function updatedServicio($val)
    {
        if ($val) {
            ...
            if ($this->tipoServicio->id == 10 || $this->tipoServicio->id == 14) {
    ...
    public function certificar()
    {
        ...
        $certi = Certificacion::certificarGnv($taller, $servicio, $hoja, $this->vehiculo, Auth::user(), $this->serviexterno, $this->placaantigua, $this->fechaCertificacion);
        ...
        if ($certi) {
            ...
            $this->emit("minAlert", ["titulo"  => "¡EXCELENTE TRABAJO!", "mensaje" => "Tu certificado N°: " . $certi->serie_certificado_completa . " con Material N°: " . $certi->Hoja->numSerie ." está listo.", "icono"   => "success"]);
        } else {
            $this->emit("minAlert", ["titulo" => "AVISO DEL SISTEMA", "mensaje" => "No fue posible certificar", "icono" => "warning"]);
        }
    }
    ...
    {
        if (empty($this->tallerAuto)) {
            $this->emit("minAlert", ["titulo" => "AVISO DEL SISTEMA", "mensaje" => "Debes seleccionar un Taller Autorizado para poder certificar.", "icono" => "warning"]);
            return;
        }
        ...
        $certi = Certificacion::certificarGlp($taller, $tallerAuto, $servicio, $hoja, $this->vehiculo, Auth::user(), $this->serviexterno, $this->fechaCertificacion);
    ...
    public function certificarmodi()
    {
        ...
        $certi = Certificacion::certificarModi($taller, $servicio, $hoja, $this->vehiculo, Auth::user(), $this->serviexterno, $this->esModiMotor, $this->fechaCertificacion);
    ...
    // Certificar PRECONVERSION GNV (PREINICIAL) ✅
    public function certificarPreconver()
    {
        ...
        if ($hoja != null) {
            if (isset($this->vehiculo)) {
                if ($this->vehiculo->esCertificableGnv) {
                    $certi = Certificacion::certificarGnvPre($taller, $servicio, $hoja, $this->vehiculo, Auth::user(), $this->serviexterno, $this->fechaCertificacion);
                    if ($certi) {
                        $this->estado = "certificado";
                        $this->certificacion = $certi;
    ...
    // Certificar PRECONVERSION GLP (PREINICIAL) ✅
    public function certificarPreconverGlp()
    {
        ...
        if ($hoja != null) {
            if (isset($this->vehiculo)) {
                if ($this->vehiculo->esCertificableGlp) {
                    $certi = Certificacion::certificarGlpPre($taller, $tallerAuto, $servicio, $hoja, $this->vehiculo, Auth::user(), $this->serviexterno, $this->fechaCertificacion);
                    if ($certi) {
                        $this->estado = "certificado";
                        $this->certificacion = $certi;
                        //$this->emit("minAlert", ["titulo" => "¡EXCELENTE TRABAJO!", "mensaje" => "Tu certificado N°: " . $certi->Hoja->numSerie . " esta listo.", "icono" => "success"]);
    ...
    */
    ...
    // Certificar CONVERSION GNV CON CHIP ✅
    public function certificarConChip()
    {
        try {
            $taller = Taller::findOrFail($this->taller);
            ...
            if ($hoja != null) {
                if (!empty($chip)) {
                    if (!empty($this->vehiculo) && $this->vehiculo->esCertificableGnv) {
                        ...
                        $certi = Certificacion::certificarGnvConChip($taller, $servicio, $hoja, $this->vehiculo, Auth::user(), $chip, $this->serviexterno, $this->fechaCertificacion);
    ...
    // CERTIFICAR CONVERSION OVERHUL GNV CON CHIP ✅
    public function certificarOverhul()
    {
        try {
            ...
            if ($hoja != null) {
                if (!empty($chip)) {
                    if (!empty($this->vehiculo) && $this->vehiculo->esCertificableGnv) {
                        $certi = Certificacion::certificarGnvOverhul($taller, $servicio, $hoja, $this->vehiculo, Auth::user(), $chip, $this->serviexterno, $this->fechaCertificacion);
    ...
    public function duplicarCertificado()
    {
        ...
        // ASIGNACIÓN Y VALIDACIÓN DE LA FECHA DE CERTIFICACIÓN(DUPLICADO)
        // Si el inspector no seleccionó fecha, asignamos la fecha actual por defecto
        if (empty($this->fechaCertificacion)) {
            $this->fechaCertificacion = Carbon::today()->toDateString();
        }
            ...
            if ($this->externo) {
                if (isset($this->vehiculo)) {
                    if ($this->vehiculo->esCertificableGnv) {
                        ...
                        $certi = Certificacion::duplicarCertificadoExternoGnv(Auth::user(), $this->vehiculo, $servicio, $taller, $hoja, $duplicado, $this->serviexterno, $this->fechaCertificacion);
                        $this->estado = "certificado";
                        $this->certificacion = $certi;
                        //$this->emit("minAlert", ["titulo" => "¡EXCELENTE TRABAJO!", "mensaje" => "Tu certificado N°: " . $certi->Hoja->numSerie . " esta listo.", "icono" => "success"]);
                        $this->emit("minAlert", ["titulo"  => "¡EXCELENTE TRABAJO!", "mensaje" => "Tu certificado N°: " . $certi->serie_certificado_completa . " con Material N°: " . $certi->Hoja->numSerie ." está listo.", "icono"   => "success"]);
                ...
                if ($this->certificado && $servicio) {
                    ...
                    $certi = Certificacion::duplicarCertificadoGnv($dupli, $taller, Auth::user(), $servicio, $hoja, $this->serviexterno, $this->fechaCertificacion);
                    $this->estado = "certificado";
                    $this->certificacion = $certi;
                    //$this->emit("minAlert", ["titulo" => "¡EXCELENTE TRABAJO!", "mensaje" => "Tu certificado N°: " . $certi->Hoja->numSerie . " esta listo.", "icono" => "success"]);
                    $this->emit("minAlert", ["titulo"  => "¡EXCELENTE TRABAJO!", "mensaje" => "Tu certificado N°: " . $certi->serie_certificado_completa . " con Material N°: " . $certi->Hoja->numSerie ." está listo.", "icono"   => "success"]);
    ...
    public function creaDuplicadoExterno()
    {
        ...
        $dupli = Duplicado::create([
            ...
            "externo" => 1,
            "fecha" => $this->fechaExterno, // fecha que el usuario digita ya que no hay una certificacion anterior a la que referenciar para sacar la fecha
        ]);
        if ($dupli) {
            return $dupli;
        } else {
    ...
    public function creaDuplicado(Certificacion $anterior)
    {
        $dupli = Duplicado::create([
            ...
            "fecha" => $anterior->fecha_documento, // fecha_documento es atributo virtual esta en protected $appends que en realidad es el campo fechaCertificado
