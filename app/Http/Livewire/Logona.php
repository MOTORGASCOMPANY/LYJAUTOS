...
class Logona extends Component
{
    ...
    public function procesar()
    {
        ...
        $this->clasificarPorFrecuencia($this->aux);
    }
    ...
    private function agruparTalleresConsolidar($tabla)
    {
        ...
        return $tabla
            ...
            ->map(function ($items) {
                ...
                // Calcular totales y porcentaje pagado
                $totalServicios = $items->count(); // Total de servicios agrupados
                $pagados = $items->where('pagado', 2)->count(); // Servicios pagados
            ...
            ->map(function ($groupedItems, $taller) {
                ...
                $totalServicios = $groupedItems->sum('total_servicios');
                $pagados = $groupedItems->sum('servicios_pagados');

                return [
                    'taller' => $taller,
                    'encargado' => $groupedItems->first()['encargado'],
                    'total' => $groupedItems->sum('total'),
                    'porcentaje_pagado' => $totalServicios > 0
                    ? round(($pagados / $totalServicios) * 100, 2)
                    : 0,
                ];
            })
            ->filter(fn ($data) => $data['total'] > 0)
