...
class ListaCertificacionesPendientes extends Component
{
    ...
    //variable para fecha certificado
    ...
    public function certificar(){
        ...
        */
        ...
        if($hoja!=null){
            ...
            $certif= Certificacion::certificarGnvPendiente($certi->Taller, $certi->Servicio, $hoja, $certi->Vehiculo, $certi->Inspector,$precio, $certi->externo, $this->fechaCertificacion);
            ...
            $certif->update(["fechaCertificado"=>$certi->created_at]);

            // Actualizar el campo pagado a 1 en la certificación
            $certif->update(['pagado' => 1]); //para que ya aparezca como generado por activacion de chips
            ...
            $this->emit("minAlert", ["titulo"  => "¡EXCELENTE TRABAJO!", "mensaje" => "Tu certificado N°: " . $certif->serie_certificado_completa . " con Material N°: " . $certif->Hoja->numSerie ." está listo.", "icono"   => "success"]);
        }else{
