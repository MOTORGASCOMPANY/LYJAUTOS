...
<body>
    ...
    <main>
        <h3>CERTIFICADO DE CONFORMIDAD DEL VEHÍCULO CON COMBUSTIÓN DE GNV</h3>
        {{--<h5>{{ 'Certificado N° ' . $hoja->numSerie . ' - ' . $fechaCert->format('Y') }}</h5>--}}
        <h5>{{ 'Certificado N° ' . $numSerie . ' - ' . $anioSerie }}</h5>
        ...
        <p>Haber efectuado la evaluación de las condiciones de seguridad respecto de la conversión del sistema de
            combustión a Gas Natural Vehicular – GNV efectuada por el taller de Conversión Autorizado: {{ $taller->nombre }}
        </p>
            ...
            <tr>
                ...
                <td style="text-align:center;">
                    ...
                    @elseif ($carro->combustible === 'DIESEL')
                        DUAL GNV
                    @elseif ($carro->combustible === 'GLP' || $carro->combustible === 'BI-COMBUSTIBLE GNV' || $carro->combustible === 'GNV')
                        GNV
                    @elseif ($carro->combustible === 'HÍBRIDO (GASOLINA/BATERIAS)')
                        HÍBRIDO (GASOLINA/BATERIAS) GNV
                    @elseif ($carro->combustible === 'HIBRIDO (DIESEL/BATERIAS)')
                        HIBRIDO (DIESEL/BATERIAS) GNV
                    @else
                        BI COMBUSTIBLE GNV
                    @endif
                </td>
        ...
        <p>Consiste por el presente documento que el sistema de combustible a Gas Natural Vehicular GNV, del vehículo
            ...
            expediente técnico N° {{ $numSerie . ' - ' . $anioSerie }}{{-- $hoja->numSerie . ' - ' . $fechaCert->format('Y') --}}, habilitándose al mismo para
            cargar Gas Natural
