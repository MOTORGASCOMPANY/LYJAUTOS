<div class="mt-12 pt-8 w-full items-center">
    <h1 class="w-full m-auto text-center font-bold text-2xl text-indigo-600">Listado de recepciones pendientes</h1>

    @if (count($recepciones))
        <div class="block m-auto pt-8 items-center justify-center w-96">
            @foreach ($recepciones as $recepcion)
                <div class="mb-4 max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow-md hover:shadow-indigo-200">
                    <div class="flex flex-row justify-between">
                        <i class="fas fa-box"></i> 
                        <span class="text-sm font-bold text-green-700">{{ $recepcion->created_at }}</span>
                    </div>
                    <a href="#">
                        <span>Codigo:</span>
                        <h5 class="mb-2 text-sm font-semibold tracking-tight text-red-700">
                            {{ $recepcion->numero }}
                        </h5>
                    </a>
                    <p class="mb-3 font-normal text-gray-500">Motivo de envio: 
                        <strong> {{ $recepcion->motivo }} </strong>
                    </p>
        ...
        <x-slot name="content">
            <div class="mt-4">
                @if ($recepcion)
                    ...
                    <p>creado por: <span class="p-1 bg-red-400 rounded-lg">{{ $recepcion->usuarioCreador->name }}</span>
                    </p>
                    <p>fecha de creación: <span class="p-1 bg-lime-400 rounded-lg">{{ $recepcion->created_at }}</span>
                    </p>
            ...
            <div class="mt-4">
                <label for="acepto" class="flex items-center">
                    ...
                    <span
                        class="ml-2 text-sm text-gray-600">{{ __('Acepto y estoy de acuerdo con la recepción de los materiales asignados en este cargo.') }}</span>
                </label>
