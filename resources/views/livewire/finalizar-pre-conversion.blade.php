<div>
        ...
        <div class="rounded-xl m-4 bg-white p-8 mx-auto max-w-max-xl shadow-lg block space-y-2 md:space-y-0 md:flex md:space-x-2">
            ...
            <div class="w-full md:w-2/6  ">
                <div class="border bg-gray-100 rounded-md p-2 w-full">
                    ...
                    <p class="text-gray-600"><i class="fa-solid fa-file-contract"></i> N° Formato: 
                        <span class="font-bold">{{ $certificacion->Hoja->numSerie }}</span>
                    </p>
                    <p class="text-gray-600"><i class="fa-solid fa-car-side"></i> VIN/serie del Vehículo: 
                        <span class="font-bold">{{ $certificacion->Vehiculo->numSerie }}</span>
                    </p>
                    <p class="text-gray-600"><i class="fa-solid fa-warehouse"></i> Taller: 
                        <span class="font-bold">{{ $certificacion->Taller->nombre }}</span>
                    </p>
                    <p class="text-gray-600"><i class="fa-solid fa-user"></i> Inspector: 
                        <span class="font-bold">{{ $certificacion->Inspector->name }}</span>
                    </p>
                </div>
            </div>
            ...
            <div class="w-full md:w-4/6 border bg-indigo-100 rounded-md h-auto p-4">
                ...
                @else
                    <p class="w-4/6 text-center p-2 text-sm text-gray-400 m-auto">No se encontraron Equipos registrados en este vehículo</p>
                @endif
                ...
                <div class="flex flex-row w-full justify-space-between space-x-2">
                    <div class="w-full md:w-2/6 flex items-center">
                        <x-jet-label value="Placa:" class="mr-2" />
                        <x-jet-input type="text" class="w-full" wire:model="vehiculo.placa" maxlength="7" />
                    </div>
                    <div class="w-full md:w-2/6 flex items-center">
                        <x-jet-label value="Año de fabricación:" class="mr-2" />
                        <x-jet-input type="text" class="w-full" wire:model="vehiculo.anioFab" maxlength="7" inputmode="numeric" pattern="[0-9]*" />
                    </div>
                    <div class="w-full md:w-2/6 flex items-center">
                        <x-jet-label value="Nuevo Precio:" />
                        <x-jet-input type="number" class="w-full" wire:model="nuevoPrecio" id="nuevoPrecio" min="0" step="0.01" required />
                    </div>                    
                    <!-- validacion para que solo aparezca para gnv y no para glp -->
                    @if ($tipoServicio == 12)
                        <div class="w-full md:w-2/6 flex justify-center items-center">
                            <x-jet-input type="checkbox" value="1" wire:model="conChip" id="checkbox1"
                                class="w-4 h-4 text-indigo-600 border-gray-300 rounded" />
                            <x-jet-label value="Incluye Chip" class="ml-2" />
                        </div>
                    @endif
                </div>
                ...
                <div class="max-w-5xl m-auto  bg-white rounded-lg shadow-md dark:bg-gray-300  mt-4 p-2">
                    ...
                    <x-file-pond name="imagenes" id="imagenes" wire:model="imagenes"
                        acceptedFileTypes="['image/*',]" aceptaVarios="true">
    ...
    <div class="hidden w-full h-screen flex flex-col justify-center items-center bg-gray-200 " wire:loading.remove.class="hidden">
        <div class="flex">
            <img src="{{ asset('images/logo.png') }}" alt="Logo Motorgas Company" width="150" height="150">
        </div>
