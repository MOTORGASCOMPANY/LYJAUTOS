<div>
    <x-jet-dialog-modal wire:model="open" wire:key="archivos-{{ $detalleId }}">
        <x-slot name="title">
            Gastos por Servicio {{ $detalle?->gastoAdministrativo?->periodo_mes . ' - ' . $detalle?->gastoAdministrativo?->periodo_anio }}
        </x-slot>
        ...
        <x-slot name="content">
            <div class="space-y-6">
                ...
                <div class="bg-gray-50 p-4 rounded-md border">
                    <h3 class="font-semibold text-sm text-gray-700">Concepto</h3>
                ...
                <div class="bg-gray-50 p-4 rounded-md border">
                    ...
                    <div class="mt-3 border-2 border-dashed border-indigo-500 rounded-lg p-6 text-center cursor-pointer hover:bg-indigo-50 transition"
                        ...
                        <input type="file" multiple wire:model.defer="files" accept="application/pdf,image/*" 
                ...
                <div class="bg-white p-4 rounded-md border shadow-sm overflow-hidden">
                    <h3 class="font-semibold text-sm mb-3 text-gray-700">
                        Archivos ({{ $detalle?->archivosPagos?->count() ?? 0 }})
                    </h3>

                    @if ($detalle?->archivosPagos?->isNotEmpty())
                        <div class="space-y-4">
                            @foreach ($detalle->archivosPagos->groupBy('tipo') as $tipo => $archivosTipo)
                                <div>
