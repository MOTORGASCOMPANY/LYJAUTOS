<div class="block justify-center mt-12 pt-8 max-h-max pb-8">
    ...
    @if ($servicio)
        @switch($tipoServicio->id)
                        ...
                        @case('esperando')
                            <div class="max-w-5xl m-auto  bg-white rounded-lg shadow-md dark:bg-gray-300 p-4 mt-4">
                                <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-4">
                                    <x-jet-label value="Fotos reglamentarias:" class="font-bold text-xl py-2 sm:py-0" />
                                    {{-- 
                                    <x-file-pond name="imagenes" id="imagenes" wire:model="imagenes" acceptedFileTypes="['image/*',]"
                                        aceptaVarios="true">
                                    </x-file-pond>
                                    --}}
                                    <input type="file" multiple wire:model="imagenes" accept="image/*"
                                        class="mt-2 sm:mt-0 flex-1 text-sm text-gray-700 border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-indigo-500 focus:border-indigo-600 cursor-pointer
                                            file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-indigo-500 file:text-white hover:file:bg-indigo-600"
                                    />
                                </div>                                
                                ...
                                @if(count($imagenes))
                                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-6">
                                        @foreach ($imagenes as $idx => $img)
                                            <div class="relative bg-gray-50 rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition duration-300">
                                                {{-- Preview de la imagen --}}
                                                <img src="{{ $img->temporaryUrl() }}" alt="preview" class="w-full h-48 object-cover rounded-t-xl">

                                                {{-- Contenedor inferior (select + confianza) --}}
                                                <div class="p-3 bg-white rounded-b-xl">
                                                    <select wire:model="tiposFotosNuevas.{{ $idx }}"
                                                        class="w-full border-gray-300 text-sm rounded-md focus:border-indigo-400 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                                        <option value="">-- Seleccione tipo --</option>
                                                        @foreach ($tipos_imagen as $tipo)
                                                            <option value="{{ $tipo->id }}">{{ $tipo->descripcion }}</option>
                                                        @endforeach
                                                    </select>

                                                    @if (isset($confidences[$idx]))
                                                        <p class="text-xs text-gray-500 mt-1 text-center">
                                                            <strong>MotorIA</strong> confianza: {{ number_format($confidences[$idx], 2) }}%
                                                        </p>
                                                    @endif
                                                </div>
                                            </div>
                            ...
                            <div class="max-w-5xl m-auto bg-white rounded-lg shadow-md my-4 py-6 px-6">
                                <div class="my-2 flex flex-col md:flex-row justify-evenly items-center">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        <x-jet-input type="checkbox" wire:model="placaantigua"
                                            class="w-4 h-4 text-indigo-600 border-gray-300 rounded" />
                                        <x-jet-label value="Placa Antigua" class="ml-2 text-sm text-gray-700" />
                                        <x-jet-input-error for="placaantigua" />
                                    </div>
                                    <div class="flex items-center space-x-2">
                                        <x-jet-input type="checkbox" wire:model="serviexterno" :checked="$inspectorexterno == true"
                                            class="w-4 h-4 text-indigo-600 border-gray-300 rounded" />
                                        <x-jet-label value="Externo" class="ml-2 text-sm text-gray-700" />
                                        ...
                                        <div x-show="menu" x-on:click.away="menu = false"
                                            ...
                                            <div class="" role="none">
                                                ...
                                                @if ($certificacion->Servicio->tipoServicio->id != 8 && $certificacion->Servicio->tipoServicio->id != 9 && $certificacion->Servicio->tipoServicio->id != 5 && $certificacion->Servicio->tipoServicio->id != 11)
                                                    ...
                                                    <a href="{{ $certificacion->rutaVistaCheckListArriba }}" target="__blank" rel="noopener noreferrer"
                                                        class="flex px-4 py-2 text-sm text-indigo-700 hover:bg-slate-600 hover:text-white justify-between items-center hover:cursor-pointer">
                                                        <i class="fas fa-eye"></i>
                                                        <span>CheckList Arriba</span>
                                                    </a>
                                                    <a href="{{ $certificacion->rutaVistaCheckListAbajo }}" target="__blank" rel="noopener noreferrer"
                            ...
                            <div class="max-w-5xl m-auto bg-white rounded-lg shadow-md my-4 py-6 px-6">
                                <div class="my-2 flex flex-col md:flex-row justify-evenly items-center">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        <x-jet-input type="checkbox" wire:model="placaantigua"
                                            class="w-4 h-4 text-indigo-600 border-gray-300 rounded" />
                                        <x-jet-label value="Placa Antigua"
                                    ...
                                    <div>
                                        <select wire:model="tallerAuto"
                                            ...
                                            @foreach ($talleres as $taller2)
                                                <!-- Mostrar solo talleres autorizados -->
                                        ...
                                        <div x-show="menu" x-on:click.away="menu = false"
                                            ...
                                            <div class="" role="none">
                                                ...
                                                @if ($certificacion->Servicio->tipoServicio->id != 8 && $certificacion->Servicio->tipoServicio->id != 9)
                                                    ...
                                                    <a href="{{ $certificacion->rutaVistaCheckListAbajo }}" target="__blank"
                                                        rel="noopener noreferrer"
                                                        class="flex px-4 py-2 text-sm text-indigo-700 hover:bg-slate-600 hover:text-white rounded-b-md justify-between items-center hover:cursor-pointer">
                                                        <i class="fas fa-eye"></i>
                                                        <span>CheckList Abajo</span>
                ...
                @if ($estado)
                    @switch($estado)
                        @case('esperando')
                            ...
                            <div class="max-w-5xl m-auto bg-white rounded-lg shadow-md my-4 py-6 px-4">
                                <div class="flex flex-col md:flex-row flex-wrap gap-16 justify-center items-center">
                                    <!-- Fecha de Certificación -->
                                    <div>
                                        <input class="form-input rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                            type="date" wire:model="fechaCertificacion" min="{{ $minDate }}" max="{{ $maxDate }}">
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        class="w-4 h-4 text-indigo-600 bg-white border-gray-300 rounded focus:ring-indigo-600 focus:ring-1 hover:cursor-pointer" />
                                        <x-jet-label value="Externo" class="ml-2 text-sm font-medium text-gray-900 select-none hover:cursor-pointer" />
                                        <x-jet-input-error for="serviexterno" />
                                    </div>
                                    <!-- Es modificacion de motor -->
                                    <div class="flex items-center">
                                        <x-jet-input type="checkbox" wire:model="esModiMotor"
                                        class="w-4 h-4 text-indigo-600 bg-white border-gray-300 rounded focus:ring-indigo-600 focus:ring-1 hover:cursor-pointer" />
                                        <x-jet-label value="¿Es modificación de Motor?" class="ml-2 text-sm font-medium text-gray-900 select-none hover:cursor-pointer" />
                                    </div>
                                    <!-- Boton certificar -->
                                    <div>
            ...
            @case(6) <!-- No lleva certificado, sin cambios -->
                @livewire('servicio-desmonte', ['servicio' => $servicio, 'taller' => $this->taller])
            @break

            @case(7) <!-- Pendiente revisar cambio para fechaCertificacion -->
            ...
            @case(8)
                ...
                @if ($externo)
                    @livewire('form-vehiculo', ['tipoServicio' => $tipoServicio])
                    <div class="max-w-5xl m-auto bg-white rounded-lg shadow-md my-4 py-4 px-8">
                        <div class="w-full block md:flex gap-4">
                            <div class="w-full mb-2">
                                <x-jet-label value="Servicio:" for="servicioExterno" class="mb-1" />
                            ...
                            <div class="w-full mb-2">
                                <x-jet-label value="Taller:" for="tallerExterno" class="mb-1" />
                                <x-jet-input type="text" wire:model="tallerExterno" class="w-full" />
                            </div>
                            <div class="w-full mb-2">
                                <x-jet-label value="Fecha Cert. Anterior:" for="fecha" class="mb-1" />
                            ...
                            <div class="c-card block bg-white shadow-md rounded-lg overflow-hidden">
                                <div class="p-4">
                                    ...
                                    <div class="my-2 flex flex row justify-between pr-4">
                                        <h2 class=" font-bold">
                                            <i class="fas fa-car"></i> &nbsp; <span class="text-indigo-600">{{ $certificado->Vehiculo->placa }}</span>
                                        </h2>
                                        <p class=" font-bold">
                                            <i class="fas fa-file"></i>&nbsp; <span class="text-red-500 font-bold">{{ $certificado->serieFormato }} - {{ $certificado->Hoja->añoActivo }}</span>
                                        </p>
                                    </div>
                                    <h3>
                                ...
                                <div class="p-4 border-t  text-xs text-gray-700">
                                    <span class="flex items-center">
                                        <i class="far fa-address-card fa-fw text-gray-900 mr-2"></i> {{ $certificado->Inspector->name }}
                                    </span>
                                    <span class="flex items-center mb-1">
                                        @if(isset($fechaCerti))
                                            <span class="flex items-center mb-1">
                                                <i class="far fa-clock fa-fw mr-2 text-gray-900"></i>
                                                certificado hace {{ $fechaCerti }} {{ $fechaCerti == 1 ? 'día' : 'días' }}.
                                            </span>
                                        @endif
                                    </span>
                                </div>
            ...
            @case(9) <!-- No existe duplicado GLP -->
                        ...
                        @case('esperando')
                            ...
                            <div class="max-w-5xl m-auto bg-white rounded-lg shadow-md my-4 py-4">
                                <div class="my-2 flex flex-col md:flex-row justify-evenly items-center">
                                    ...
                                    <div>
                                        <button wire:click="certificarConChip" wire:loading.attr="disabled" wire:target="certificarConChip"
                                            class="hover:cursor-pointer border border-indigo-500 focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600 sm:mt-0 inline-flex items-center justify-center px-6 py-3 bg-indigo-400 hover:bg-indigo-500 focus:outline-none rounded">
                                            <p class="text-sm font-medium leading-none text-white">
                                                <span wire:loading wire:target="certificarConChip">
                                                    <i class="fas fa-spinner animate-spin"></i>
                                                    &nbsp;
                                                </span>
                                                &nbsp;Certificar
                        ...
                        @case('esperando')
                            ...
                            <div class="max-w-5xl m-auto bg-white rounded-lg shadow-md my-4 py-4">
                                <div class="my-2 flex flex-col md:flex-row justify-evenly items-center">
                                    ...
                                    <div>
                                        <button wire:click="certificarOverhul" wire:loading.attr="disabled" wire:target="certificarOverhul"
                                            class="hover:cursor-pointer border border-indigo-500 focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600 sm:mt-0 inline-flex items-center justify-center px-6 py-3 bg-indigo-400 hover:bg-indigo-500 focus:outline-none rounded">
                                            <p class="text-sm font-medium leading-none text-white">
                                                <span wire:loading wire:target="certificarOverhul">
                                                    <i class="fas fa-spinner animate-spin"></i>
                                                    &nbsp;
                                                </span>
                                                &nbsp;Certificar
