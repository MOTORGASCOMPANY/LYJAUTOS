<div>
                                    ...
                                    <tbody>
                                        @foreach ($certificaciones as $item)
                                            <tr>
                                                ...
                                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                                    <div class="flex items-center">
                                                            <p class="p-2 border  rounded-md font-black text-md">
                                                                @if ($item->Servicio->tipoServicio->id == 11)
                                                                    @if (is_null($item->descripcion))
                                                                        {{ Str::afterLast($item->UbicacionHoja, '/') ?? 'N.A' }}
                                                                    @else
                                                                        {{ Str::afterLast($item->descripcion, '/') }}
                                                                    @endif                                                                    
                                                                @else
                                                                    {{ $item->placa ?? 'N/A' }}
                                                                @endif
                                                ...
                                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                                    <div class="flex items-center justify-center"
                                                        ...
                                                        <div x-show="menus" x-on:click.away="menus = false"
                                                            class="dropdown-content flex flex-col border bg-white shadow-xl w-48 absolute z-30 right-0 mt-24 mr-48">
                                                            ...
                                                            @if ($item->Servicio->tipoServicio->id == 1 || $item->Servicio->tipoServicio->id == 10 
                                                                || $item->Servicio->tipoServicio->id == 12 || $item->Servicio->tipoServicio->id == 14)
                                                            ...
                                                            @if ($item->Servicio->tipoServicio->id != 8 && $item->Servicio->tipoServicio->id != 9 && $item->Servicio->tipoServicio->id != 5 && $item->Servicio->tipoServicio->id != 11)
                                                ...
                                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                                    <div class="flex items-center justify-center" x-data="{ menu: false }">                                                        
                                                        <button
                                                        ...
                                                        <div x-show="menu" x-on:click.away="menu = false"
                                                            ...
                                                            {{-- Finalizar pre-conversion aun le falta la logica de numSerie y anioSerie del certificado 
                                                                @if ($item->Servicio->tipoServicio->id == 12 && $item->estado == 3 || $item->Servicio->tipoServicio->id == 13 && $item->estado == 3)
                                                            ...
                                                            <button wire:click="solicitarAnulacion({{ $item->id }})"
                                                                class="focus:outline-none flex items-center space-x-4 focus:text-lime-400 text-xs w-full hover:bg-indigo-600 py-2 px-6 cursor-pointer hover:text-white">
                                                                <i class="fa-solid fa-file-circle-xmark"></i>
                                                                <span>Solicitar anulacion</span>
                                                            </button>

                                                            <button wire:click="solicitarEliminacion({{ $item->id }})"
                                                                class="focus:outline-none flex items-center space-x-4 focus:text-lime-400 text-xs w-full hover:bg-indigo-600 py-2 px-6 cursor-pointer hover:text-white">
    ...
    <x-jet-dialog-modal wire:model="modal">
        <x-slot name="title">
            <h1 class="text-xl font-bold">Solicitar Anulación y Sustituir</h1>
        </x-slot>
        <x-slot name="content">
            <div>
                <p class="text-sm text-gray-600 mb-4">
                    Nota: Solo puede enviar solicitudes de anulación dentro de los 7 días posteriores a la creación de la certificación.
                </p>
                <div class="grid grid-cols-3 gap-4 mb-4">
                    ...
                    <div class="col-span-1">
                        <x-jet-label class="block mt-2 text-sm font-medium text-gray-600" value="Nuevo N° Formato:" />
                        <x-jet-input type="text" id="nuevoNumSerie" wire:model="nuevoNumSerie" class="w-full mt-1 form-input"/>
                        <x-jet-input-error for="nuevoNumSerie" />
                    </div>
                </div>
                <div class="mb-4">
        ...
        <x-slot name="footer">
            <x-jet-secondary-button wire:click="$set('modal',false)" class="mx-2">
                Cancelar
