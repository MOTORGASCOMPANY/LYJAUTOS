@php
    ...
    $cumpleañosProximos = User::whereBetween(DB::raw('DATE_FORMAT(cumpleanos, "%m-%d")'), [
        $hoy->format('m-d'),
                    ...
                    <div class="block py-2 text-base text-gray-500">
                        ...
                        <ul>
                            @foreach ($cumpleañosProximos as $empleado)
                                <li class="mt-2">
                                    🎂 {{ $empleado->name ?? null }}
                                    📅 {{ \Carbon\Carbon::parse($empleado->cumpleanos)->translatedFormat('d') }} de
                                    {{ \Carbon\Carbon::parse($empleado->cumpleanos)->translatedFormat('F') }} del
