...
use App\Http\Livewire\ActualizarExternos;
use App\Http\Livewire\ActualizarFechas;
use App\Http\Livewire\ActualizarPrecios;
use App\Http\Livewire\ActualizarPreciosGasol;
use App\Http\Livewire\ActualizarResumenDeben;
use App\Http\Livewire\Admin\AsistenciaDashboard;
use App\Http\Livewire\Admin\AsistenciaReportes;
use App\Http\Livewire\Admin\DispositivosManager;
use App\Http\Livewire\Admin\HorariosManager;
...
use App\Http\Livewire\AsignarHorario;
...
use App\Http\Livewire\ConfiguracionCandado;
use App\Http\Livewire\ConsultarHoja;
use App\Http\Livewire\Contabilidad\ListaFacturasContabilidad;
use App\Http\Livewire\ContratosTrabajos;
use App\Http\Livewire\ControlFormatos;
use App\Http\Livewire\DetalleVacaciones;
use App\Http\Livewire\EditarBoleta;
use App\Http\Livewire\EditarContados;
...
use App\Http\Livewire\GastosAdministrativos\GastosAdministrativosForm;
use App\Http\Livewire\GastosAdministrativos\GastosAdministrativosIndex;
use App\Http\Livewire\Gratificaciones\ListaGratificaciones;
...
use App\Http\Livewire\InformeCilindros;
...
use App\Http\Livewire\ListaContados;
...
use App\Http\Livewire\MarcadoAsistencia;
use App\Http\Livewire\Memorandos;
use App\Http\Livewire\NotificacionesPendientes;
use App\Http\Livewire\Organigramas;
use App\Http\Livewire\PlanillaArchivos;
use App\Http\Livewire\PlanillaArchivosEmpleados;
...
use App\Http\Livewire\RentabilidadResumen;
use App\Http\Livewire\ReporteContados;
use App\Http\Livewire\ReporteDocumentosEmpleados;
...
// Rutas para QR (ver PDF) Inicial-GNV OVERHUL
Route::get('/certificado-inicial-gnv-overhul/{id}/temp', [PdfController::class, 'generaPdfInicialGnvOverhul'])->name("verPdfInicialOverhul");
...
Route::middleware(['auth:sanctum', config('jetstream.auth_session'), 'verified'])
    ->group(function () {
        ...
        Route::get('/Expedientes', Expedientes::class)->middleware('can:expedientes')->name('expedientes'); // Lista y subir imagenes
        ...
        Route::get('/Reporte-de-documentos-a-vencer', ReporteDocumentosTaller::class)->name('reportes.reporteDocumentosTaller'); // reporte de documentos taller
        Route::get('/Reporte-documentos-empleados', ReporteDocumentosEmpleados::class)->name('reportes.reporteDocumentosEmpleados'); // reporte de documentos empleados

        // RUTAS PARA VER LOS REPORTES DE SERVICIOS COMPLETO Y DETALLADO
        Route::get('/ReporteCalcular', ReporteCalcularGasol::class)->middleware('can:reportes.reporteCalcularGasol')->name('reportes.reporteCalcularGasol'); //1 Reporte externo resumen + Reporte taller resumen = completo
        Route::get('/ReporteCalcular-taller', ReporteCalcularChip::class)->middleware('can:reportes.reporteCalcularChip')->name('reportes.reporteCalcularChip'); //2 Reporte taller detallados
        Route::get('/Reporte-MTG', ReportesMtg::class)->middleware('can:reportes.reporteMTG')->name('reportes.reporteMTG'); //3 Reporte detallado MTG mejorar descarga excell
        Route::get('/Reporte-semanal', ReporteCalcular::class)->middleware('can:reportes.reporteCalcular')->name('reportes.reporteCalcular'); //4 Reporte Semanal
        
        // RUTAS PARA VER LOS REPORTES DE SERVICIOS RESUMEN
        Route::get('/Reporte-taller-resumen', Logona::class)->middleware('can:reportes.reporteTallerResumen')->name('reportes.reporteTallerResumen'); //1 Reporte taller resumen        
        Route::get('/Reporte-ExternosMTG', Cambiar::class)->middleware('can:reportes.reporteExternosMTG')->name('reportes.reporteExternosMTG'); //2 Reporte externos resumen        
        ...
        Route::get('/Reporte-deben', ResumenDeben::class)->middleware('can:reportes.resumenDeben')->name('reportes.resumenDeben'); // Resumen los que faltan por cobrar
        Route::get('/Reporte-deudas', ResumenDeudasBoletas::class)->middleware('can:reportes.resumenDeudas')->name('reportes.resumenDeudas');
        

        //RUTAS PARA COONSULTA DE MATERIALES Y OTRAS ACCIONES (ACTUALIZAR PRECIO, FECHA Y PAGADOS)
        ...
        Route::get('/Actualizar-precio-mtg', ActualizarPrecios::class)->middleware('can:reportes.reporteActualizarPrecio')->name('reportes.reporteActualizarPrecio'); //6 Reporte Actualizar precios MTG
        Route::get('/Actualizar-precio-gasol', ActualizarPreciosGasol::class)->middleware('can:reportes.reporteActualizarGasol')->name('reportes.reporteActualizarGasol'); //7  Reporte Actualizar precios GASOL
        Route::get('/Actualizar-deuda', ActualizarResumenDeben::class)->middleware('can:reportes.resumenActualizar')->name('reportes.resumenActualizar');
        Route::get('/Actualizar-fecha', ActualizarFechas::class)->middleware('can:reportes.fechaActualizar')->name('reportes.fechaActualizar');       
        Route::get('/Actualizar-Cert-Externo', ActualizarExternos::class)->middleware('can:reportes.externoActualizar')->name('reportes.externoActualizar');       


        // RUTAS PARA VER LOS FORMATOS AL CONTADO
        Route::get('/ListaContados', ListaContados::class)->name('ListaContados');
        Route::get('/FormatosContados', ReporteContados::class)->name('FormatosContados');
        Route::get('/Contados/{idContado}', EditarContados::class)->name('editar-contados');

        //RUTA PARA RENTABILIDAD DE TALLERES O EXTERNOS
        Route::get('/Rentabilidad', Rentabilidad::class)->middleware('can:Rentabilidad')->name('Rentabilidad');
        Route::get('/Rentabilidad-resumen', RentabilidadResumen::class)->middleware('can:RentabilidadResumen')->name('RentabilidadResumen');

        // RUTA PARA REGISTRAR GASTOS ADMINISTRATIVOS
        Route::get('/GastosAdministrativos', GastosAdministrativosIndex::class)->middleware('can:GastosAdministrativos')->name('GastosAdministrativos');
        Route::get('/GastosAdministrativos/{id}', GastosAdministrativosForm::class)->name('gastos-administrativos.form');

        //RUTA PARA CONTROL DE FORMATOS
        Route::get('/Controlformatos', ControlFormatos::class)->middleware('can:Controlformatos')->name('Controlformatos');

        // CANDADO PARA ANULACIONES Y ELIMINACIONES
        Route::get('/Candado', ConfiguracionCandado::class)->middleware('can:Candado')->name('Candado');
        ...
        // PARA INFORME DE CILINDROS
        Route::get('/InformeCilindros', InformeCilindros::class)->name('InformeCilindros');

        // LISTAR Y SUBIR FACTURAS CONTABILIDAD (COMPRA Y VENTA)
        Route::get('/Contabilidad', ListaFacturasContabilidad::class)->name('Contabilidad');
        ...
        Route::get('/detalle-vacaciones', DetalleVacaciones::class)->name('detalleVacaciones');

        // RUTA PARA MOSTRAR Y CREAR PLANILLA
        Route::get('/Planilla', Planillas::class)->middleware('can:Planillas')->name('Planillas');
        Route::get('/planilla-comprobantes', PlanillaArchivosEmpleados::class)->name('planilla.comprobantes');

        // RUTA PARA GRATIFICACIONES
        Route::get('/Gratificaciones', ListaGratificaciones::class)->name('Gratificaciones');
        ...
        Route::controller(PdfController::class)->group(function () {
            ...
            Route::get('/certificado-inicial-overhul/{id}', 'generaPdfInicialGnvOverhul')->name("certificadoInicialGnvOverhul"); //inicial overhul
            ...
            Route::get('/certificado-inicial-overhul/{id}/descargar', 'descargarPdfInicialGnvOverhul')->name("descargarCertificadoInicialGnvOverhul");
            ...
            Route::get('/preConversionGnv/{idCert}', 'generarPreConversionGnv')->name("preConversionGnv"); // evaluacion pre conversion GNV
            ...
            //Rutas para descargar certificado preconversion GNV (PREINICIALES)
            ...
            //Rutas para descargar y ver documentos complementarios de GLP
            ...
            //Rutas para descargar certificado preconversion GLP (PREINICIALES)
            ...
            Route::get('/contratopla/{id}', 'generaPdfContratoPlanilla')->name("contratoPlanilla");
            ...
            //Rutas para comprobante formatos contado
            Route::get('/contado/{id}', 'generaPdfContado')->name("generaPdfContado");
            ...
            Route::get('/planillas/{periodo}', 'generaPdfPlanilla')->name("planillas.pdf");

            Route::get('/resumen-vacaciones','generaPdfresumenVacaciones')->name('pdf.resumen.vacaciones');
        });
        ...
        // Para que sirven estas rutas (revisar)
